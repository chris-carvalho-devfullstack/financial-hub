...
// Certifique-se de ter as imagens em public/logos/
const ALL_PLATFORMS = [
  { 
    id: Platform.UBER, 
    label: 'Uber', 
  ...
  { 
    // Usamos 'as Platform' para forçar o TS a aceitar, já que não está no enum original
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="flex-1 w-full md:max-w-2xl">
          ...
          <form onSubmit={handleSave} className="space-y-6 md:space-y-8">
            ...
            {/* PLATAFORMA (GRID - DESIGN DE ÍCONE DE APP) */}
            <div>
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {displayedPlatforms.map((p) => {
                     ...
                     return (
                       <button
                         ...
                         className={`
                           relative h-28 rounded-2xl border transition-all duration-200 flex flex-col items-center justify-center gap-2 group overflow-hidden active:scale-95
                           ${isSelected 
                             ? `${p.bg} ${p.textColor} border-transparent shadow-lg ring-2 ring-white/30` 
                             : 'bg-gray-900 border-gray-800 text-gray-400 hover:bg-gray-800'}
                         `}
                       >
                          {/* LOGO CONTAINER:
                             - overflow-hidden: Corta o excesso
                             - object-cover: Preenche TODO o espaço (sem borda branca)
                          */}
                          <div className={`
                            w-12 h-12 rounded-xl flex items-center justify-center overflow-hidden
                            ${!p.logo ? 'bg-white/10' : 'bg-white'} 
                            ${isSelected ? 'shadow-lg scale-110' : 'opacity-90 group-hover:scale-110 transition-transform'}
                          `}>
                            {p.logo ? (
                              <img 
                                src={p.logo} 
                  ...
                  {!showAllPlatforms && (
                    <button
                      ...
                      className="h-28 rounded-2xl border border-gray-800 bg-gray-900/50 hover:bg-gray-800 text-gray-400 hover:text-white transition-all flex flex-col items-center justify-center gap-2 group active:scale-95"
                    >
                      <div className="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center group-hover:bg-gray-700">
                        <LayoutGrid size={20} />
                      </div>
                  ...
                  {showAllPlatforms && (
                    <button
                      ...
                      className="h-28 rounded-2xl border border-dashed border-gray-700 bg-transparent text-gray-500 hover:text-white hover:border-gray-500 transition-all flex flex-col items-center justify-center gap-1 active:scale-95"
                    >
                      <ChevronUp size={24} />
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div className="col-span-1 md:col-span-2">
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-gray-950 border border-emerald-500/30 rounded-xl py-4 pl-12 md:pl-14 text-white text-2xl md:text-3xl font-bold focus:ring-2 focus:ring-emerald-500 outline-none placeholder-emerald-900/30 h-16 md:h-20"
                          placeholder="0,00"
                        />
                      </div>
                  </div>
                  <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-gray-950 border border-gray-700 rounded-xl p-3 md:p-4 text-white focus:border-emerald-500 outline-none h-12 md:h-14"
                      />
                  </div>
               </div>
            </div>
                ...
                <div className="bg-gray-900/50 p-3 rounded-xl border border-gray-800 flex flex-col justify-center">
                   ...
                   <div className="flex items-center gap-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-transparent text-white font-bold outline-none border-b border-gray-700 focus:border-blue-500 pb-0.5 text-sm md:text-base"
                        placeholder="0"
                      />
                   </div>
                </div>
                ...
                <div className="bg-gray-900/50 p-3 rounded-xl border border-gray-800 flex flex-col justify-center">
                   ...
                   <div className="flex items-center gap-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-transparent text-white font-bold outline-none border-b border-gray-700 focus:border-yellow-500 pb-0.5 text-sm md:text-base"
                        placeholder="0.0"
                      />
                   </div>
                </div>
                ...
                <div className="bg-gray-900/50 p-3 rounded-xl border border-gray-800 flex flex-col justify-center">
                   ...
                   <div className="flex items-center gap-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-transparent text-white font-bold outline-none border-b border-gray-700 focus:border-purple-500 pb-0.5 text-sm md:text-base"
                        placeholder="0"
                      />
                   </div>
                </div>
            ...
            <button 
              type="submit" 
              disabled={saving}
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-emerald-900/20 transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 text-base md:text-lg h-14 md:h-16"
            >
        ...
        <div className="flex-1 w-full md:border-l md:border-gray-800 md:pl-8">
           ...
           <div className="mt-8 md:mt-[112px]">
              <div className="flex items-center justify-between mb-4">
                 <h3 className="text-gray-400 font-bold uppercase text-sm flex items-center gap-2">
                    <History size={16} /> Histórico Recente
                 </h3>
                 {recentGains.length > 0 && (
                   <span className="text-[10px] text-gray-500 bg-gray-900 px-2 py-1 rounded border border-gray-800 font-mono">
                     {recentGains.length} últimos
                   </span>
                 )}
              </div>
              ...
              {loading ? (
                <div className="space-y-3 animate-pulse">
                   {[1,2,3].map(i => <div key={i} className="h-16 bg-gray-900 rounded-lg"></div>)}
                </div>
              ) : recentGains.length === 0 ? (
                <div className="text-center py-10 opacity-50 border border-dashed border-gray-800 rounded-xl bg-gray-900/30">
                ...
                <div className="space-y-3">
                   {recentGains.map(gain => {
                      ...
                      return (
                        <div key={gain.id} className="group relative bg-gray-900 border border-gray-800 hover:border-gray-600 p-3 rounded-xl transition-all flex items-center justify-between overflow-hidden active:bg-gray-800">
                           <div className="flex items-center gap-3">
                              {/* Logo Miniatura no Histórico (Preenchida) */}
                              <div className="w-10 h-10 min-w-[2.5rem] rounded-xl bg-white flex items-center justify-center shadow-sm overflow-hidden">
                                {platformInfo.logo ? (
                                   <img src={platformInfo.logo} alt={platformInfo.label} className="w-full h-full object-cover" />
                                ) : (
                              ...
                              <div>
                                 ...
                                 <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
                                    ...
                                    {gain.distanceDriven > 0 && (
                                       <span className="flex items-center gap-0.5 border-l border-gray-700 pl-2">
                                          <Map size={10}/>{gain.distanceDriven}km
                                       </span>
                                    )}
                                 </div>
                              </div>
                           ...
                           <div className="flex items-center gap-3">
                              ...
                              <button 
                                ...
                                disabled={isDeleting}
                                className="p-2 text-gray-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors md:opacity-0 md:group-hover:opacity-100"
                                title="Excluir"
                              >
